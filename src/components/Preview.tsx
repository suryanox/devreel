...
export default function Preview() {
  const { parsedSchema, currentScene, setCurrentScene } = useReelStore();
  const [isPlaying, setIsPlaying] = useState(false);
  const [sceneProgress, setSceneProgress] = useState(0);
  const intervalRef = useRef<ReturnType<typeof setInterval> | null>(null);
  const progressRef = useRef(0);

  const scenes = parsedSchema?.scenes ?? [];
  ...
  useEffect(() => {
    ...
    intervalRef.current = setInterval(() => {
      ...
      if (progressRef.current >= sceneDuration) {
        ...
        setCurrentScene((prev: number) => {
          const next = prev + 1;
          if (next >= totalScenes) { stopPlayback(); return prev; }
          return next;
        });
      }
    }, tick);
    ...
    <div className="preview-panel">
        ...
        <div className="phone-frame">
          ...
          {parsedSchema && (
            <div className="scene-progress-bar">
              <div className="scene-progress-fill" style={{ width: `${sceneProgress * 100}%` }} />
            </div>
          )}
      ...
      {parsedSchema && (
        <div className="preview-controls">
          ...
          <button className="ctrl-btn" onClick={() => goToScene(currentScene - 1)} disabled={currentScene === 0}>
            <ChevronLeft size={16} />
          </button>
          <button className="ctrl-btn ctrl-btn--play" onClick={togglePlay}>
            {isPlaying ? <Pause size={16} /> : <Play size={16} />}
          </button>
          <button className="ctrl-btn" onClick={() => goToScene(currentScene + 1)} disabled={currentScene === totalScenes - 1}>
            <ChevronRight size={16} />
          </button>
          <span className="scene-counter">{currentScene + 1} / {totalScenes}</span>
        </div>
      )}

      {/* Scene dots */}
      {parsedSchema && totalScenes > 1 && (
