...
export default function Circuit() {
  ...
  useEffect(() => {
    ...
    // Generate a circuit trace from a random start point
    ...
    const draw = () => {
      ...
      // Dark bg with subtle fade
      ctx.fillStyle = "rgba(5, 13, 8, 0.92)"
      ...
      tracesRef.current.forEach((trace, idx) => {
        ...
        // Draw the animated pulse segment
        ...
        if (endT > startT) {
          ...
          ctx.beginPath()
          // Re-walk the path from startT to endT
          let drawing = false
          let accumulated = 0
          const total = getTotalLength(points)
        ...
        // Draw corner nodes (junction dots)
        points.forEach((pt, i) => {
        ...
        if (trace.progress > 1.25) {
          tracesRef.current[idx] = generateTrace()
        }
      })
      ...
      vignette.addColorStop(1, "rgba(5,13,8,0.7)")
